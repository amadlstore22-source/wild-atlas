      ...
      <div className="min-h-screen">
        <div className="relative h-[55vh] min-h-[360px] overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col justify-end max-w-4xl mx-auto px-4 sm:px-6 pb-12 w-full left-0 right-0">
            ...
            <h1 className="font-serif text-white text-5xl sm:text-6xl lg:text-7xl font-bold leading-tight mb-4">{post.title}</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-14">
            <article className="bg-white rounded-3xl p-8 sm:p-12 shadow-sm">
              <p className="text-xl text-charcoal/70 leading-relaxed mb-8 font-medium border-l-4 border-sunset pl-5">{post.excerpt}</p>
              ...
              <div className="mt-10 pt-8 border-t border-sand-dark flex flex-wrap gap-2">
                <Tag className="w-4 h-4 text-charcoal/40 mt-0.5" />
                {post.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 bg-sand rounded-full text-xs text-charcoal/60 font-medium">{tag}</span>
                ))}
              </div>
            </article>
            ...
            <aside className="space-y-6">
              <div className="bg-forest rounded-2xl p-6 text-white sticky top-24">
                <h3 className="font-serif text-xl font-bold mb-2">{dict.tourDetail.planYourTrip}</h3>
                <p className="text-white/70 text-sm mb-5 leading-relaxed">{dict.tourDetail.sidebarDesc}</p>
                <Link href={`/${lang}/tours`} className="block text-center px-5 py-3 rounded-full bg-sunset text-white font-semibold text-sm hover:bg-orange-600 transition-colors mb-3">
              ...
              {relatedPosts.length > 0 && (
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  <h3 className="font-serif text-lg font-bold text-charcoal mb-4">{dict.tourDetail.relatedArticles}</h3>
                    ...
                    {relatedPosts.map((related) => (
                      <Link key={related.slug} href={`/${lang}/blog/${related.slug}`} className="flex gap-3 group">
                        ...
                        <div>
                          ...
                          <p className="text-charcoal/40 text-xs mt-1 flex items-center gap-1"><Clock className="w-3 h-3" /> {related.readTime} {dict.blog.minRead}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
          ...
          {(prev || next) && (
            <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {prev && (
                <Link href={`/${lang}/blog/${prev.slug}`} className="flex items-center gap-4 bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow group">
                  <ArrowLeft className="w-5 h-5 text-charcoal/40 group-hover:text-forest transition-colors shrink-0" />
                  <div>
                    <p className="text-xs text-charcoal/40 uppercase tracking-wide mb-1">{dict.tourDetail.previous}</p>
                    <p className="text-charcoal font-semibold text-sm group-hover:text-forest transition-colors leading-snug">{prev.title}</p>
                  </div>
                </Link>
              )}
              {next && (
                <Link href={`/${lang}/blog/${next.slug}`} className="flex items-center gap-4 bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow group sm:ml-auto sm:text-right">
                  <div>
                    <p className="text-xs text-charcoal/40 uppercase tracking-wide mb-1">{dict.tourDetail.next}</p>
                    <p className="text-charcoal font-semibold text-sm group-hover:text-forest transition-colors leading-snug">{next.title}</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-charcoal/40 group-hover:text-forest transition-colors shrink-0" />
                </Link>
              )}
            </div>
          )}
