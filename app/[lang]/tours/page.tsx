...
type ToursPageProps = {
  ...
  searchParams: Promise<{ q?: string; origin?: string; cat?: string; diff?: string }>;
};
...
export default async function ToursPage({ params, searchParams }: ToursPageProps) {
  ...
  const { q = "", origin = "", cat = "", diff = "" } = await searchParams;

  return (
    <ToursClient lang={lang} dict={dict} initialSearch={q} initialOrigin={origin} initialCategory={cat} initialDifficulty={diff} />
  );
}
