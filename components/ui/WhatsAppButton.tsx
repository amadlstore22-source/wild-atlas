...
export default function WhatsAppButton({ dict }: { dict: Dictionary }) {
  ...
  // Tour detail pages always render the sticky booking bar, which carries its
  // own WhatsApp button — two of the same control, one covering the other.
  // Checked by route as well as by DOM: the route test is true on first paint,
  // before any effect runs, so the float never flashes over the bar.
  const onTourPage = /^\/[a-z]{2}\/tours\/[^/]+/.test(pathname);
  const suppressed = barPresent || onTourPage;
