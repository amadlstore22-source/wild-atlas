  ...
  return (
    <section className="py-24 bg-offwhite">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {featured.map((guide, i) => {
            const initials = guide.name
              ...
              .map((n) => n[0])
              .join("");
            const accentColor = i === 0 ? "#2B3A67" : "#C97B2B";

            return (
              <Link
                key={guide.id}
                href={`/${lang}/guides/${guide.id}`}
                className="group bg-card rounded-[4px] border border-sand-dark overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col sm:flex-row"
              >
                {/* Avatar column */}
                <div
                ...
                <div className="p-7 flex flex-col justify-between flex-1 min-w-0">
                  <div>
                    <h3 className="font-display text-charcoal text-xl font-bold mb-1 group-hover:text-forest transition-colors">
                      {guide.name}
                    </h3>
                    <p className="text-ink-soft text-xs mb-4">{guide.role}</p>
                    <p className="text-ink-soft text-sm leading-relaxed line-clamp-3">{guide.shortBio}</p>
                  </div>
        ...
        {/* Legacy callout */}
        <div className="mt-8 bg-charcoal rounded-[4px] p-7 flex flex-col sm:flex-row items-start sm:items-center gap-5">
